...
PROFIT_TARGET_PCT  = 0.20   # take profit at +20%
TRAILING_STOP_PCT  = 0.05   # trailing stop: sell if falls 5% from peak
MIN_BUY_SCORE      = 22     # minimum composite score to enter

TRADES_FILE   = Path("docs/trades.json")
PEAK_FILE     = Path("docs/peaks.json")    # tracks per-position peak price
...
def ai_sentiment(ticker):
    """Score news sentiment -10 to +10 using Claude Haiku."""
    ...
    try:
        ...
        logger.debug(f"Sentiment error for {ticker}: {e}")
        return 0
...
def get_tradeable(candidates):
    """Keep only Alpaca-active, fractionable US equities."""
    try:
...
def _extract(daily, hourly):
    ...
    if hourly is not None and "Close" in hourly.columns:
        ...
        if len(hc) >= 26:
            ...
            e26 = _ema(hc, 26)
            if e21:
                ema_cross = (e9 - e21) / e21 * 100
            if e26:
                macd_val = (e12 - e26) / e26 * 100
...
def fetch_batch(tickers):
    """Download all tickers in parallel chunks via yfinance batch API."""
    if not tickers:
        return {}
...
def score(tk, d, sentiment=0):
    ...
    daily momentum, intraday momentum, volume, RSI, EMA cross,
    MACD, Bollinger position, 5-day range position, AI sentiment.
    """
    s     = 10   # base
    ...
    # Bollinger position (buy sweet spot 40-75)
    if   40 < bb < 75: s += 10
    elif bb >= 75:     s +=  4
    elif bb < 20:      s -= 8
...
def calc_notional(portfolio_val, buying_power, price, atr):
    """ATR-based risk sizing: risk 1% of portfolio with 2×ATR stop."""
...
def run():
    ...
    # ── SELL: stop loss / trailing stop / profit target ───────────────────
    for sym, pos in list(held.items()):
        try:
            ...
            if pnl_pct <= -(STOP_LOSS_PCT * 100):
                ...
                reason = f"trailing stop (peak ${peak:.2f} → ${current:.2f}, {trail_drop:.1f}%)"
            ...
            if reason:
                ...
                logger.info(f"HOLD {sym} — P&L {pnl_pct:+.1f}% | peak ${peak:.2f} | trail {trail_drop:.1f}%")
        except Exception as e:
            logger.warning(f"Sell check error {sym}: {e}")

    # ── BUY: score → AI sentiment → buy top candidates ────────────────────
    ...
    if open_slots <= 0:
        ...
        # Top 12 by technical score → run AI sentiment on those
        ...
        # AI sentiment for top candidates (only if API key exists)
        final_scores = []
        for tk, tech_sc in candidates_buy:
            ...
            if final_sc >= MIN_BUY_SCORE:
                ...
                logger.info(f"  {tk}: tech={tech_sc} sentiment={sent:+.1f} final={final_sc}")

        final_scores.sort(key=lambda x: -x[1])

        if not final_scores:
            logger.info(f"No tickers passed score {MIN_BUY_SCORE} after AI sentiment filter.")
        else:
            for tk, sc, sent in final_scores[:open_slots]:
                try:
                    ...
                    logger.info(
                        ...
                        f"| stop ${stop_price} | score {sc} | sentiment {sent:+.0f}"
                    )
                    alpaca_post("/v2/orders", {
                        "symbol":        tk,
