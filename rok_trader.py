...
def fetch_live_data(tickers):
    ...
    for tk in tickers:
        try:
            ...
            # Intraday momentum: last bar vs 4 bars ago
            intraday_mom = 0.0
            ...
            # RSI (14-period) using hourly bars
            rsi = 50.0
...
def signal_score(tk, d):
    ...
    # Intraday momentum over last 4 hours (max ±20)
    ...
    # RSI sweet spot 55–74: momentum without being overbought (max ±15)
...
def run():
    ...
    # Verify connectivity + market status
    try:
    ...
    for tk, sc in top[:open_slots]:
        try:
            ...
            if price <= 0:
                continue
            # Use notional (dollar amount) so fractional shares work for any price
